  ...
  group('ObservableCollection', () {
    group('flatMapCollectionAsList', () {
      ObservableList<int> createResultList({
        ...
        return base.flatMapCollectionAsList<int>(
          sourceProvider: (final ObservableSetChange<int> change) {
            final Set<int> added = change.added;
            final Set<int> removed = change.removed;

            final Map<int, ObservableList<int>> newObservables = <int, ObservableList<int>>{};

            for (final int key in added) {
              if (key == 1) {
                newObservables[key] = source1;
              } else if (key == 2) {
                newObservables[key] = source2;
              }
            }

            return ObservableCollectionFlatMapUpdate<int, int, ObservableList<int>>(
    ...
    group('flatMapCollectionAsMap', () {
      group('Map to map', () {
        test('Should listen on changes from maps', () async {
          ...
          final ObservableMap<String, int> result = set.flatMapCollectionAsMap<String, int>(
            sourceProvider: (final ObservableSetChange<int> change) {
              final Set<int> added = change.added;
              final Set<int> removed = change.removed;

              final Map<int, ObservableMap<String, int>> newObservables = <int, ObservableMap<String, int>>{};
              ...
              return ObservableCollectionFlatMapUpdate<int, String, ObservableMap<String, int>>(
                newObservables: newObservables,
                removedObservables: removed,
              );
            },
